...
extractBasesMT=/cbica/projects/GURLAB/microbiome/scripts/extractBasesMT
...
if [ $? != 0 ] ; then
	...
	echo "${date_info} : Failure to execute extractBasesMT, JobID: ${JOB_ID}, method: ${method}, number of bases: ${num_bases}" >>/cbica/projects/GURLAB/microbiome/output/logs/FailedExtractBasesExperimentsMT.txt
	exit 1
fi
